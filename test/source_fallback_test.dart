...
void main() {
  test(
    'sourceFallback builds native_socket from the checked out source tree',
    () async {
      final root = Directory.current;
      final temp = await Directory.systemTemp.createTemp(
        'native_socket_source_fallback',
      );
      try {
        ...
        await PrebuiltCodeAssetBuilder(
          ...
          input: input,
          output: output,
          logger: Logger('native_socket_test'),
        );

        final built = output.build();
        expect(built.assets.code, hasLength(1));
        expect(
          File.fromUri(built.assets.code.single.file!).existsSync(),
          isTrue,
        );
      } finally {
