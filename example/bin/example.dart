...
Future<void> demoBindConnectAccept() async {
  ...
  final path =
      '$tmpDir/native_socket_demo_${DateTime.now().microsecondsSinceEpoch}.sock';
...
Future<void> demoDatagram() async {
  ...
  final tmpDir = Directory.systemTemp.path;
  final path =
      '$tmpDir/native_socket_dgram_demo_${DateTime.now().microsecondsSinceEpoch}.sock';
...
Future<void> demonativeBuffer() async {
  ...
  // Heap-backed buffer
  final heapBuf = NativeBuffer.fromList(
  ...
  print(
    '  Native buffer: ${nativeBuf.asTypedList().take(2).toList()} ... (${nativeBuf.length} bytes)',
  );
  nativeBuf.free();
...
Future<void> demoUnlink() async {
  ...
  final tmpDir = Directory.systemTemp.path;
  final path =
      '$tmpDir/native_socket_cleanup_demo_${DateTime.now().microsecondsSinceEpoch}.sock';
